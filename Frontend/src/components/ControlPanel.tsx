...
interface ControlPanelProps {
  darkMode: boolean;
  config: any;   // Added: Live config from MongoDB
...
export function ControlPanel({ darkMode, config, apiUrl }: ControlPanelProps) {
  ...
  // Sync internal device state with MongoDB config prop whenever it changes
  useEffect(() => {
    if (config) {
  ...
  // NEW: Function to update MongoDB through the Backend
  ...
  const toggleDevice = (id: string) => {
    ...
    // Map frontend IDs to your specific backend Schema fields
